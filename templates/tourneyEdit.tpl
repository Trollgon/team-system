{capture assign='contentHeader'}
    <header class="contentHeader articleContentHeader">
        <div class="contentHeaderTitle">
            <h1 class="contentTitle" itemprop="name headline">{lang}tourneysystem.tourney.settings.header{/lang}: {@$tourney->tourneyName}</h1>
        </div>
        ...
        {if $__wcf->getUser()->getUserID() != 0}
            {hascontent}
                <nav class="contentHeaderNavigation">
                    <ul>
                        {content}
                        {if $__wcf->getSession()->getPermission('admin.tourneySystem.canCreateRulebooks')}
                            <li><a href="{link application='tourneysystem' controller='RulebookAdd' isACP=true}{/link}"
                                   title="{lang}wcf.acp.menu.link.tourneysystem.rulebook.add{/lang}" class="button"><span
...
<form method="post" action="{link application='tourneysystem' controller='TourneyEdit' object=$tourney}{/link}">
    <fieldset>
